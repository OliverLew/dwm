		...
		if (i < LENGTH(tags)) {
			...
			while (text[++i]) {
				if ((unsigned char)text[i] < ' ') {
					...
					i = -1;
					if (x >= ev->x) break;
					if (ch < LENGTH(statuscmds)) statuscmdn = ch;
				}
			}
		} else
			click = ClkWinTitle;
...
cyclelayout(const Arg *arg) {
	static int lts[LENGTH(layouts)] = {0};
	int curtag = selmon->pertag->curtag;

	lts[curtag] += arg->i;
	if (lts[curtag] < 0)
		lts[curtag] = LENGTH(layouts) - 1;
	if (lts[curtag] > LENGTH(layouts) - 1)
		lts[curtag] = 0;
	setlayout(&((Arg) { .v = &layouts[lts[curtag]] }));
}
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	wc.border_width = c->bw;
	if (((!gapsingle && nexttiled(c->mon->clients) == c && !nexttiled(c->next))
...
setgaps(const Arg *arg)
{
	if ((arg->i == 0) || (selmon->gappx + arg->i < 0))
		...
		selmon->gappx += arg->i;
	selmon->pertag->gappxs[selmon->pertag->curtag] = selmon->gappx;
	arrange(selmon);
}
